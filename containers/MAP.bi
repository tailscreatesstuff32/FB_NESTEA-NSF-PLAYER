...
#macro MMapTemplate(ctkey , ctdata , MapParameters...)
	...
	'			Declare Function FindNode overload(pNode As MAPNODE##ctkey##ctdata Ptr , nKey As ctkey) As Zstring Ptr
				'Declare Function FindNode OverLoad(pNode As MAPNODE##ctkey##ctdata Ptr , nKey As ctkey) As MAPNODE##ctkey##ctdata Ptr
				
			#else
								Declare Function FindNode (pNode As MAPNODE##ctkey##ctdata Ptr , nKey As ctkey) As ctdata

				'Declare Function FindNode OverLoad(pNode As MAPNODE##ctkey##ctdata Ptr , nKey As ctkey) As ctdata
				'Declare Function FindNode OverLoad(pNode As MAPNODE##ctkey##ctdata Ptr , nKey As ctkey) As MAPNODE##ctkey##ctdata Ptr
		...
		End Function
			...
			'Function TMAP##ctkey##ctdata.FindNode(pNode As MAPNODE##ctkey##ctdata Ptr , nKey As ctkey) As MAPNODE##ctkey##ctdata Ptr
			'	
			'	If pNode = 0 Then
			'		
			'		Return 0
	...
	End Function	
			...
			''#if ctdata = String
			''	
			''	Dim As MAPNODE##ctkey##ctdata Ptr nodetemp = FindNode(pRoot , nKey)
			''	
			''	If nodetemp Then Return nodetemp
			''	
			''#else
			''	
			''	Return FindNode(pRoot , nKey)
